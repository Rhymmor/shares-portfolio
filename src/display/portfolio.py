...
def display_categories_distrib_table(distrib: SharesDistribution):
    categories = distrib.categories \
        if isinstance(distrib.shares, list) \
        else map_funds_distribution_to_categories(distrib.shares)
    ...
    df = pd.DataFrame(np.array([categories_types_column, values_column]).transpose(), columns=['', ''], index=index)
    display(df)
