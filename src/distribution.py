...
def create_distributions_from(
    default_distrib: SharesDistribution,
    create_fn: Callable[[SharesDistribution, T], None],
    data: List[T],
):
...
def modify_distrib(
    default_distrib: SharesDistribution,
    modify_fn: Callable[[SharesDistribution, T], None],
    arg: T,
):
...
def calc_distributions_data(
    ...
    distributions_returns = [
        calc_portfolio_returns(x, SHARES_DATA, from_year=from_year)[0] for x in distributions_by_fund
    ]
    ...
    return DistributionsData(
        ...
        annual_mean_returns=[
            round(calc_annual_geometric_mean_from_monthly(x), 2) for x in distributions_returns  # type: ignore
        ],
